...
- WebSocket proxy to `ws://brewos.local/ws` (ESP32) or `ws://localhost:3001/ws` (cloud)
- API proxy to `http://brewos.local/api` (ESP32) or `http://localhost:3001/api` (cloud)
...
# Output: ../firmware/src/esp32/data/
```
...
This app repository is separate from the firmware repository. When building for ESP32:
- Set `ESP32_DATA_DIR` environment variable to point to firmware's ESP32 data directory
- The firmware repository will use this built app for deployment
