...
export default defineConfig(({ mode, command }) => {
  ...
  // ESP32 output directory - configurable via env var for external repo builds
  // Default assumes app repo is sibling to firmware repo
  const esp32DataDir = process.env.ESP32_DATA_DIR || "../firmware/src/esp32/data";
  ...
  return {
    ...
    build: {
      outDir: isEsp32 ? esp32DataDir : "dist",
      emptyOutDir: true,
